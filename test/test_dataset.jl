...
@testset "Assign labels blank" begin
  @test AdaSubGram.Dataset.assign_labels!(["hello", "my", "name", "is", "jeff"], Dict{String, UInt64}(), Vector{String}(undef, 100), UInt64(1)) == ([1, 2, 3, 4, 5], 6)
  @test AdaSubGram.Dataset.assign_labels!(["that", "is", "that"], Dict{String, UInt64}(), Vector{String}(undef, 100), UInt64(1)) == ([1, 2, 1], 3)
end

@testset "Assign labels with existing" begin
  @test AdaSubGram.Dataset.assign_labels!(["hello", "my", "name", "is", "jeff"], Dict{String, UInt64}("hello" => 3, "jeff" => 1, "is" => 2), ["jeff", "is", "hello", "", ""], UInt64(4)) == ([3, 4, 5, 2, 1], 6)
  @test AdaSubGram.Dataset.assign_labels!(["that", "is", "that"], Dict{String, UInt64}("this" => 1, "is" => 2), ["this", "is", ""], UInt64(3)) == ([3, 2, 3], 4)
end
...
@testset "Assign labels updates maps" begin
  ...
  AdaSubGram.Dataset.assign_labels!(["hello", "my", "name", "is", "jeff"], string_to_index, index_to_string, UInt64(4))
